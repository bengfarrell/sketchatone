...
# webapp, no systemd service). Udev rules for tablet /dev/hidraw* access
# are installed by the postinst via sketchatone-ui-setup-autostart
# --install-hid; the interactive tty1 autostart remains opt-in.

set -e

# Self-locate: this script lives under install-scripts/ and references both
# sibling helper scripts and project-root paths (python/, public/, dist/).
...
if [ -f "$SCRIPT_DIR/setup-ui-autostart.sh" ]; then
    cp "$SCRIPT_DIR/setup-ui-autostart.sh" "$PKG_DIR/usr/bin/sketchatone-ui-setup-autostart"
...
if [ -f "$SCRIPT_DIR/setup-pi-boot-trim.sh" ]; then
    cp "$SCRIPT_DIR/setup-pi-boot-trim.sh" "$PKG_DIR/usr/bin/sketchatone-ui-boot-trim"
...
if [ -f "$SCRIPT_DIR/sketchatone-ui-configure" ]; then
    cp "$SCRIPT_DIR/sketchatone-ui-configure" "$PKG_DIR/usr/bin/sketchatone-ui-configure"
    chmod +x "$PKG_DIR/usr/bin/sketchatone-ui-configure"
    echo "📦 Including interactive configure menu"
fi
...
Description: Sketchatone (Kivy UI variant)
 ...
 compositor required). No web UI, no systemd service.
 .
...
echo "📦 Installing sketchatone package (no-deps; deps installed above)..."
...
# Grant /dev/hidraw* access non-interactively. Uses the same helper as
# the interactive autostart flow but limited to udev rules + group
# membership: no tty1 autologin, no bash_profile edits. Guarded with
# `|| true` so a failure here (missing helper, unusual layout) never
# breaks the whole install.
if [ -x /usr/bin/sketchatone-ui-setup-autostart ]; then
    /usr/bin/sketchatone-ui-setup-autostart --install-hid || true
fi
...
echo ""
echo "Optional setup (interactive menu covers tty1 autostart and boot-time"
echo "trimming):"
echo "  sudo sketchatone-ui-configure"
echo ""
exit 0
POSTINSTEOF
chmod +x "$PKG_DIR/DEBIAN/postinst"

# prerm: remove the udev rules we installed, but only on true removal.
# dpkg calls prerm with "upgrade"/"failed-upgrade" during upgrades, and
# we want the rules to persist across those; only "remove" and "purge"
...
case "$1" in
    remove|purge)
        if [ -x /usr/bin/sketchatone-ui-setup-autostart ]; then
            /usr/bin/sketchatone-ui-setup-autostart --uninstall-hid || true
        fi
        ;;
esac
